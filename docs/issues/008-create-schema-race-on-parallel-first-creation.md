...
**Status:** ✅ RESOLVED — принят минимальный фикс (вариант 4.1 ниже): `CREATE SCHEMA IF NOT
EXISTS` в `sql_asset.go:92`. Релиз v1.2.13. Узкое окно неатомарности каталога PG при этом
остаётся теоретически, но проявляется только на первом создании схемы — принято приемлемым.
