...
- The wrapper must stay portable across engines: alias the subquery (`) as teal_test_src`) and repeat the aggregate in `HAVING` instead of referencing the `test_count` output alias — PostgreSQL resolves neither
